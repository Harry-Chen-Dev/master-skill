# B2B 大客户销售 (Enterprise B2B Sales) Master OS — Synthesis

> Phase 2 提炼结果。Phase 3 (skill writeup) 直接消化本文件。research date: 2026-06-11。
> 本行业 NOT cold:正典、podcast、第一方数据极丰;真正的边界是 en-primary + 厂商研究商业动机(见 §8)。
...
> 这行资深人拿到新问题(一个 deal、一个 territory、一个组织设计)时的标准做功课路径。6 维度,互斥:各自产出不同结论。
...
- **输出格式**: 缺口清单(字段→证据状态→补证动作),附 qualify/disqualify 建议一句话。
...
- **输出格式**: 干系人矩阵(姓名→角色→温度→最近触点)+ 「下一个该触达的人 + 借谁的手」。
...
- **输出格式**: 「你在 X 派语境,这个任务该调用 Y 层 + Z 框架」的组合建议,附该框架的已知批判一句话(防银弹化)。
...
- **输出格式**: 犹豫类型判定(估值问题/信息过载/结果恐惧)+ 对应 JOLT 动作(给推荐/限选项/去风险条款)。
...
- **输出格式**: 合规 checklist(渠道→地域→当前规则→动作)+ paper process 预案(步骤→负责人→典型时长),标注「TCPA 类条目须复核至今日判例」。
...
- **输出格式**: 选型/使用建议 + decay 警告(「此结论基于 2026-06 工具格局,SaaS 层 6 个月后须复查」)。
...
- **Source counts**: 六轨 manifest 共 229 个信息源;first-hand(verified_primary 155 + surrogate_primary 39)= 84.7%(source_manifest 公开统计,0 blacklisted / 0 dead / 0 violation)。
- **Cumulative findings issues**(已裁决,无阻塞矛盾):
  - Keenan/Blount 部分 verbatim 经 Goodreads(secondary)聚合——引用为逐字原文,已在卡内标注「建议书原文复核页码」;
  - Voss 谱系归属裁决为「谈判子技能」而非全流程 OS(§7 第 7 派照此表述);
  - Cegelski & Farrokh(30MPC)按「现代战术层/信息源」处理而非单一思想家(§5/sources 高亮,不入心智模型 figures 链);
  - Skok 按「销售经济学地基」挂入 1.6/1.7,不作人物 sub-skill(intake skip_sub_skills=true,本轮无 sub-skills);
  - Track 05 新 figure 候选 Andy Paul(反教条席位)记入候选池,本轮不展开。
