# UBNT/UniFi Enterprise Wi-Fi Engineering Master OS — Synthesis

> Phase 2 提炼结果。Phase 3 (SKILL.md writeup) 直接消化本文件。
> Synthesis date: 2026-06-02. Locale: global (EN primary, zh-CN gap acknowledged).
> Scenario anchor: 80-person dense office (Phase 4 sanity test target — see §1.1 / §2 rule 1 / §4 W3).

---

## 1. 心智模型 (6 个 — 在 3-7 目标范围内)
...
## 2. 标准 Playbook (8 条 — 在 5-10 目标范围内)
...
## 4. 工作流 / Pipeline (sanity-check 摘要)
...
## 9. Agentic Protocol — 研究维度 (6 个互斥)

> 推导自 §1 心智模型. §B 互斥性硬规矩自检通过 — 6 维度独立产出不同结论.
...
- **输出格式**: 结构化 → `客户端密度: {N 人 × M 设备 × P% 并发 = K active radios}; 每 AP 容量: {100-200 Mbps × 30-60% 效率 = Q Mbps (业内估计 — T01-S007 vonNagy/T04-CWDP 公开公式)} ; 推荐 AP 数: {K / (Q ÷ per-client SLA) × 1.5 safety = R 个}; 与现有部署 gap: {±X 个}`
...
- **输出格式**: 结构化 → `症状层级 (PHY/MAC/assoc/auth/IP): {层}; retry %: {N% — 健康/亚健康/失败}; MCS: {高/中/低 — 是否退到 802.11g 速率}; channel utilization: {N%}; SNR: {N dB at client desk}; 假设根因: {CCI/noise/client driver/auth retry}`
...
- **输出格式**: 结构化 → `客户端总数: {N}; Wi-Fi 代际分布: {ax:N1%, ac:N2%, n:N3%}; 11r 安全启用: {是/否, 风险设备 list}; WPA3 兼容: {N% 现代 + IoT 排他 SSID 计划}; 弱设备约束: {最差 Tx power / 最差 chipset 限制设计上限}`
...
- **输出格式**: 结构化 → `共信道 AP 计数: {N (target ≤ 3 在 5 GHz)}; 信道宽度: {推荐 5 GHz 40 MHz dense / 80 MHz sparse, 2.4 GHz 20 MHz only}; DFS 信道启用: {是/否, 雷达检测频率}; 非 Wi-Fi 干扰: {蓝牙/微波/雷达 — 频段 + 时段}; 信道重规划建议: {AP 1 → ch X, AP 2 → ch Y, ...}`
...
- **输出格式**: 结构化 → `当前版本: {Network App vX.Y, AP firmware vA.B}; 与最新 gap: {N versions behind}; 未 patch 的 CVE: {CVE-202X-XXXX list with severity}; 协议层风险: {WPA2 fallback / PMF Optional / 11w 未 Mandatory}; 建议升级窗口: {immediate CVE-critical / next maintenance window feature-only}`
...
- **输出格式**: 结构化 → `客户对比的 vendor: {Mist / Meraki / Aruba / ...}; 客户在意 dimension: {AIOps / cloud / 成本 / 自管 / 全球}; UBNT 相对优势: {成本 5-10x 便宜 + 自管 controller + Hosted UniFi 选项}; UBNT 相对劣势: {AIOps 弱 + 大型企业 reference 案例少 + AIOps cert 体系 (JNCIS-MistAI) 不存在}; 妥协路径: {如客户坚持 AIOps → unifi-mcp + unpoller + Claude "poor man's AIOps", 但成本与体验差距 transparent 告知}`

### §B 互斥性自检
- 9.1 (容量) ⊥ 9.4 (CCI): 容量看 demand-side 客户端 × throughput, CCI 看 supply-side AP 邻居数. **不重叠** ✓
- 9.2 (症状) ⊥ 9.4 (CCI): 症状看 client retry %, CCI 看 RF spectrum 占用. 互补但 **不重叠** ✓
- 9.3 (客户端) ⊥ 9.5 (CVE): 客户端 audit 是 client OS/chipset, CVE 是 vendor firmware/spec. **不重叠** ✓
- 9.6 (vendor 对比) 独立于 9.1-9.5: 仅在客户提 Mist/Meraki 时触发, 与场景诊断正交. **不重叠** ✓
- 6 维度各能产出独立结论 — 验证通过.
...
- **Source counts**:
  ...
  - **Cumulative**: ~232 source_id references, ~180 unique
- **Cumulative findings issues / cross-track contradictions**: 无未解决矛盾 (Phase 1.5 review checkpoint 通过). 最强共识: "vendor-neutral RF physics 是基础" (15/15 figures 共识). 最强分歧: AIOps 派 (Friday) vs vendor-neutral CWNE 阵营 (Akin/Parsons/Badman) — 保留在 §7 智识谱系, 不抹平.
- **Candidate counts**: 19 mental model candidates → 6 retained; 13 playbook candidates → 8 retained; 12 anti-pattern candidates → 8 retained; 6 Agentic Protocol candidates → 6 retained (验证全部互斥).
- **Voice confidence**: medium-high (14 voice samples × 4 类, ≥ 8 段下限远超; 但缺 (原话) 标志段 — 全为 (转述) / (推断))
- **Phase 2.5 review checkpoint**: 待执行 — 摘要建议优先 confirm: (a) 6 个 mental model 的覆盖度, (b) 8 个 playbook 是否覆盖 80 人办公场景, (c) §7 三大分歧的保留 vs 取一边, (d) §8 voice 维度的诚实声明 register.

## 质量自检 (提交前)

- [x] Step 0 候选清单 19 mental model / 13 playbook / 12 anti-pattern / 6 Agentic Protocol — 超 floor (≥ 15) ✓
- [x] `synthesis-candidates.md` 已写 ✓
- [x] Step 1 心智模型 6 个 (3-7 范围) — 每个有 evidence ≥ 2 source_ids 跨 ≥ 2 figures + 局限 ✓
- [x] Step 2 playbook 8 条 (5-10 范围) — 每条 "**如果 X**, 则 Y" 句式 + 具体案例 + evidence ✓
- [x] Step 3 工具栈 sanity check 通过 (必备 8 / 场景 10 / 新兴 4, 决策树 13 节点) ✓
- [x] Step 4 工作流 sanity check 通过 (8 workflows, W3 = 80-人 USER SCENARIO 重点标注) ✓
- [x] Step 5 表达 DNA 含 outsider-tell top 10 + 厂商话术拒绝 top 5 + 对话样本 14 段 ≥ 8 段下限 ✓
- [x] Step 6 质量基准 4 条 + 反模式 8 条 ✓
- [x] Step 7 智识谱系 6 流派 + 3 大分歧保留 ✓
- [x] Step 8 诚实边界 6 条 (≥ 3 floor) + 数字带 source 自检 ✓
- [x] Step 9 Agentic Protocol 6 维度 + §B 互斥性自检通过 + 每维 "看什么/在哪看/输出格式" 三段填了 ✓
- [x] 元数据段有 synthesis date + source counts ✓

**最终输出**: 6 mental models / 8 playbook rules / 6 Agentic Protocol dimensions / 6 schools of thought / voice_confidence medium-high / 6 honest boundary statements / 80-person dense office 场景 在 §1.1 (MM1) + §2 rule 1+2 + §4 W3 + §9.1 反复 anchor. 无未解决跨 track 矛盾.
