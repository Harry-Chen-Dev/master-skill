...
WORKFLOW="审查与备案流程 (China Censorship & Filing Process)"

usage() {
  cat <<EOF
china-censorship-filing-process.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}

逐步走完这个工作流的入门 SOP, 然后做失败模式自检, 输出走查报告。

USAGE:
  ./china-censorship-filing-process.sh             # interactive
  ./china-censorship-filing-process.sh --dry-run
  ./china-censorship-filing-process.sh --json
  ./china-censorship-filing-process.sh --explain
  ./china-censorship-filing-process.sh --help
EOF
}

explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "审查与备案流程 (China Censorship & Filing Process)" 工作流。

一句话: 中国影视项目从备案到公映/上线的法定审批全流程, 是中国编剧日常第一约束
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "china-censorship-filing-process-walkthrough"
fi
