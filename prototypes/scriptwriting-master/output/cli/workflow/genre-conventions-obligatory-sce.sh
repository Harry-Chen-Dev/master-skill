...
WORKFLOW="类型片剧作与类型自觉 (Genre Conventions & Obligatory Scenes)"

usage() {
  cat <<EOF
genre-conventions-obligatory-sce.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}

逐步走完这个工作流的入门 SOP, 然后做失败模式自检, 输出走查报告。

USAGE:
  ./genre-conventions-obligatory-sce.sh             # interactive
  ./genre-conventions-obligatory-sce.sh --dry-run
  ./genre-conventions-obligatory-sce.sh --json
  ./genre-conventions-obligatory-sce.sh --explain
  ./genre-conventions-obligatory-sce.sh --help
EOF
}

explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "类型片剧作与类型自觉 (Genre Conventions & Obligatory Scenes)" 工作流。

一句话: 在确定类型后, 用类型惯例与必备场景搭骨架, 先满足观众的情感承诺再谈创新
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "genre-conventions-obligatory-sce-walkthrough"
fi
