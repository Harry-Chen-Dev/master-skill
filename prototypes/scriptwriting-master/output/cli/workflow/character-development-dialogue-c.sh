...
WORKFLOW="人物开发与对白工艺 (Character Development & Dialogue Craft)"

usage() {
  cat <<EOF
character-development-dialogue-c.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}

逐步走完这个工作流的入门 SOP, 然后做失败模式自检, 输出走查报告。

USAGE:
  ./character-development-dialogue-c.sh             # interactive
  ./character-development-dialogue-c.sh --dry-run
  ./character-development-dialogue-c.sh --json
  ./character-development-dialogue-c.sh --explain
  ./character-development-dialogue-c.sh --help
EOF
}

explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "人物开发与对白工艺 (Character Development & Dialogue Craft)" 工作流。

一句话: 将纸板角色开发成有 want/need/flaw/wound 的立体人物, 并用潜台词/意图-阻碍写出有张力的对白
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "character-development-dialogue-c-walkthrough"
fi
