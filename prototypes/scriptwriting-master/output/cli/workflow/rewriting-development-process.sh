...
WORKFLOW="改稿与开发 (Rewriting & Development Process)"

usage() {
  cat <<EOF
rewriting-development-process.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}

逐步走完这个工作流的入门 SOP, 然后做失败模式自检, 输出走查报告。

USAGE:
  ./rewriting-development-process.sh             # interactive
  ./rewriting-development-process.sh --dry-run
  ./rewriting-development-process.sh --json
  ./rewriting-development-process.sh --explain
  ./rewriting-development-process.sh --help
EOF
}

explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "改稿与开发 (Rewriting & Development Process)" 工作流。

一句话: 从初稿 + notes 出发, 找到 note behind the note, 做分层改稿 (polish/rewrite/page-one rewrite) 的核心工艺
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "rewriting-development-process-walkthrough"
fi
