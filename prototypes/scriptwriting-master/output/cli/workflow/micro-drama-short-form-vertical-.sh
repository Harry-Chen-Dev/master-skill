...
WORKFLOW="微短剧/竖屏短剧创作 (Micro-Drama / Short-Form Vertical Drama)"

usage() {
  cat <<EOF
micro-drama-short-form-vertical-.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}

逐步走完这个工作流的入门 SOP, 然后做失败模式自检, 输出走查报告。

USAGE:
  ./micro-drama-short-form-vertical-.sh             # interactive
  ./micro-drama-short-form-vertical-.sh --dry-run
  ./micro-drama-short-form-vertical-.sh --json
  ./micro-drama-short-form-vertical-.sh --explain
  ./micro-drama-short-form-vertical-.sh --help
EOF
}

explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "微短剧/竖屏短剧创作 (Micro-Drama / Short-Form Vertical Drama)" 工作流。

一句话: 竖屏微短剧 (每集 1-3 分钟, 80-100 集) 的强钩子/爽点/反转密集型创作流程, 与长剧集工艺差异极大
...
declare -a SOP_STEPS=(
  "定爽点母题与人设 — 选验证过的爽点母题 (逆袭/打脸/重生/虐恋/赘婿/战神/复仇) + 强人设 (身份反差大), 微短剧靠即时情绪爽感 → 2. 前 6 集极限前置钩子 — 第一集前几十秒必须抛出最强冲突/反差/悬念 (黄金前 6 分钟/前 3 集决定生死), 慢热=用户划走 → 3. 设计充值点/付费点 — 在情绪最高/悬念最强处设充值解锁 (通常前若干集免费, 高潮卡点付费), 把付费模型反向嵌进结构 → 4. 高密度反转写分集脚本 — 每集至少一个反转/爽点/钩子, 集尾必留悬念, 用中文脚本格式+密集节奏标注写 80-100 集 → 5. 投流测试与迭代 — 上线后看完播率/付费转化/弃剧点数据, 据此调整钩子/爽点节奏 (微短剧是数据驱动的快速迭代生产) evidence: [T03-S012, T03-S019, T03-S025]"
)
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "micro-drama-short-form-vertical--walkthrough"
fi
