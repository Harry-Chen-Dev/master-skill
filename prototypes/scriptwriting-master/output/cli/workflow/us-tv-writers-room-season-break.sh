...
WORKFLOW="美剧编剧室运作 (US TV Writers' Room Season Break)"

usage() {
  cat <<EOF
us-tv-writers-room-season-break.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}

逐步走完这个工作流的入门 SOP, 然后做失败模式自检, 输出走查报告。

USAGE:
  ./us-tv-writers-room-season-break.sh             # interactive
  ./us-tv-writers-room-season-break.sh --dry-run
  ./us-tv-writers-room-season-break.sh --json
  ./us-tv-writers-room-season-break.sh --explain
  ./us-tv-writers-room-season-break.sh --help
EOF
}

explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "美剧编剧室运作 (US TV Writers' Room Season Break)" 工作流。

一句话: Showrunner 领导下多名编剧集体 break 整季故事, 分配分集, 统稿改写的协作制度
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "us-tv-writers-room-season-break-walkthrough"
fi
