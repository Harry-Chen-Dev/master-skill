...
WORKFLOW="IP 改编 (Adaptation: Novel/Comic/Game to Screen)"

usage() {
  cat <<EOF
ip-adaptation-novel-comic-game-t.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}

逐步走完这个工作流的入门 SOP, 然后做失败模式自检, 输出走查报告。

USAGE:
  ./ip-adaptation-novel-comic-game-t.sh             # interactive
  ./ip-adaptation-novel-comic-game-t.sh --dry-run
  ./ip-adaptation-novel-comic-game-t.sh --json
  ./ip-adaptation-novel-comic-game-t.sh --explain
  ./ip-adaptation-novel-comic-game-t.sh --help
EOF
}

explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "IP 改编 (Adaptation: Novel/Comic/Game to Screen)" 工作流。

一句话: 从原著 IP (小说/网文/漫画/游戏/真实事件) 找到戏剧脊柱, 做媒介转换与压缩取舍, 改编成可拍剧本
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "ip-adaptation-novel-comic-game-t-walkthrough"
fi
