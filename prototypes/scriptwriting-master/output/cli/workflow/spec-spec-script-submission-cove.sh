...
WORKFLOW="Spec 剧本投递与评估 (Spec Script Submission & Coverage)"

usage() {
  cat <<EOF
spec-spec-script-submission-cove.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}

逐步走完这个工作流的入门 SOP, 然后做失败模式自检, 输出走查报告。

USAGE:
  ./spec-spec-script-submission-cove.sh             # interactive
  ./spec-spec-script-submission-cove.sh --dry-run
  ./spec-spec-script-submission-cove.sh --json
  ./spec-spec-script-submission-cove.sh --explain
  ./spec-spec-script-submission-cove.sh --help
EOF
}

explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "Spec 剧本投递与评估 (Spec Script Submission & Coverage)" 工作流。

一句话: 无人脉编剧写 spec 剧本, 通过竞赛/平台/经纪人体系入行的投递与评估流程
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "spec-spec-script-submission-cove-walkthrough"
fi
