...
explain() {
  cat <<'EOT'
这个决策助手把 台词 主题下的 playbook 规则组合成交互树。

基于 1 条规则:
  • 如果 对白太直白 (on the nose) → 则 : 则加潜台词层 — 让角色说的和想的不一样。检测: 能不能给这段对白加括号写「(其实他想说的是...)」? 写不出 =
...
declare -a RULE_ACTIONS=(
  ": 则加潜台词层 — 让角色说的和想的不一样。检测: 能不能给这段对白加括号写「(其实他想说的是...)」? 写不出 = 没潜台词。案例: 《教父》开场求婚礼恩情 evidence: [T04-S028, T06-S005]"
)
