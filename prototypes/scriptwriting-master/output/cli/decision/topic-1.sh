...
explain() {
  cat <<'EOT'
这个决策助手把 类型 主题下的 playbook 规则组合成交互树。

基于 2 条规则:
  • 如果 类型片缺必备场景 → 则 : 则观众会觉得被骗 — 列出该类型的 obligatory scenes,逐一检查。悬疑必有「真相揭露」; 爱情必有「
  • 如果 想颠覆类型 → 则 : 则先完全掌握该类型的惯例再打破 — 不懂规则的「颠覆」不是颠覆,是不专业。案例: 《惊声尖叫》(Scream) 先遵
...
declare -a RULE_ACTIONS=(
  ": 则观众会觉得被骗 — 列出该类型的 obligatory scenes,逐一检查。悬疑必有「真相揭露」; 爱情必有「黑暗时刻+重逢」。案例: Snyder 《救猫咪》对 10 类的必备场景清单 evidence: [T04-S013, T01-S010]"
  ": 则先完全掌握该类型的惯例再打破 — 不懂规则的「颠覆」不是颠覆,是不专业。案例: 《惊声尖叫》(Scream) 先遵守再解构恐怖片 evidence: [T04-S013, T01-S022]"
)
