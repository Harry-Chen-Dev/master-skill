# Synthesis — Traditional Chinese Medicine (中医诊疗)

> Phase 2 — 提炼 6-track research 笔记为可运行的「中医诊疗 Master OS」。本提炼是 2026-05-18 截止的横切快照。中医基础理论 + 经典 + 流派 + 师承体系 1-2 年才动 (稳定层); 监管 + 中医诊所备案 + NMPA 中药注册 + 临床路径 + 国际化 WHO ICD-11 实施 6-12 月可能 1-2 次调整 (政策层); 智能辅助诊疗 + 中医 AI + 古代经典名方 后续 + Cochrane SR 累积 + 海外执照动向 6-12 月持续更新 (工具+证据层)。临床警示绝不软化 — 急症西医优先 + 中药安全意识 + 假中医严格区分 是 任何 master OS 不可妥协的底线。
...
中医师 面对一个新问题 (新患者 / 新病种 / 新方案) 时, 会按以下 **7 个维度** 做功课:
...
- **输出格式**: 病案 ZY/T 001 标准格式 — 望/闻/问/切 全要素 + 辨证 全过程 + 病机分析 + 治则 + 选方 + 加减 + 嘱托 + 随访计划.
...
- **输出格式**: 处方 (君臣佐使 + 用量 + 引药 + 加减依据) + 配伍禁忌 (十八反 + 十九畏) 检查 + 用法 用量 + 煎服 + 注意事项 + 监测计划 + 双签字 + 病案归档.
...
- **输出格式**: 五重审查 检查表 (适应症 + 禁忌 + 剂量 + 疗程 + 中西药相互作用) + 含 已知风险成分 必标 + informed consent + 替代品 优先 + 长期 监测计划 + 患者教育.
...
- **输出格式**: 病性 + 病势 + 病程 评估 + 主导决定 (西医主导 / 中医主导 / 双主导) + 中医介入时机 + 协同方案 + 监测 + 多学科会诊 + 病案 全记录 + 与西医基础药 不冲突 检查.
...
- **输出格式**: 每个推荐 标 evidence level (Cochrane SR 一级 / RCT 一级 / RWS 二级 / 协会指南 三级 / 名老中医共识 三级 / 单家经验 四级 / 古籍 五级 + 现代验证) + 来源 引用 (URL + DOI + 期刊 + 名老中医 学术继承人 项目编号 + 古籍 卷次篇章).
...
- **输出格式**: 资质 + 病案 + 处方 + 院感 + 监管 合规检查表 + 不超 NMPA 注册范围 + 不开 已取消药用标准成分 + 治未病服务 在 NATCM 服务规范内 + 远程开方 + 跨境用药 严守当地法律.
...
- **输出格式**: 海外执业 当地法律 + 当地执照 + 当地药物可获得性 + 当地责任保险 + 当地责任纠纷案例 + 跨境用药 严守 当地 + 中国大陆 双重监管 + 不跨境销售药品 + 远程开方 严守 当地处方权.

---

# 附: 关键 source 索引 (research 笔记完整)

- 6 轨 research manifest: `references/research/01-figures.md` (66 sources / 27 figures) + `02-tools.md` (38) + `03-workflows.md` (50 + 15 SOPs) + `04-canon.md` (60) + `05-sources.md` (45) + `06-glossary.md` (128) = **387 一手 source 行 (100% 一手率 本 prototype metadata — 250 verified_primary + 137 surrogate_primary, 0 blacklisted, 0 secondary)**.
- 27 figures + 9 流派 + 7 心智模型 + 10 playbook + 5 场景特化 + 5 完整 SOP + 12 voice 段 + 5 质量基准 + 10 反模式 + 9 边界 + 7 Agentic 维度.
- 反例 严控 不软化: 张悟本 / 胡万林 / 倪海厦 — 全部 一手 = 卫生部 + NMPA + 法院判决 + 国际医学界 学术批判, 严守 学术批评不入人身攻击.
- 临床警示 不软化: 马兜铃酸肾病 (NEJM 1993) + 何首乌 (NMPA 2014) + 雷公藤 + 中药注射剂 (鱼腥草 2006 + 双黄连 + 清开灵 + 喜炎平) + 重金属 (朱砂 雄黄) + 妊娠/儿童/老人 禁忌 — 五处 不可妥协 anchor.
