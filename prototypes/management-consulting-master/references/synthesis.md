# 管理咨询 (Management Consulting) Master OS — Synthesis

> Phase 2 提炼结果。Phase 3 (skill writeup) 直接消化本文件。
> locale = global / **en-primary canon**；profile = practitioner。research last_checked = 2026-06-09；synthesis date = 2026-06-09。
> 第一性张力：**假设驱动（answer-first / Day-1 answer，先射箭再画靶）⇄ 数据驱动（data-led，先穷尽事实再归纳）**——资深人偏 top-down 假设以减少 boil-the-ocean，但必须时刻防 confirmation bias（这条「纪律 vs 滥用」是全行业最深的张力）。更深两层：**卖洞见（so-what）⇄ 卖工时（analysis 体力）**、**客户影响（落地）⇄ 智识严谨（漂亮 deck）**。
> 跨 track 矛盾未掩盖：定位学派（Porter，可固守优势）⇄ 应急战略（Mintzberg，战略涌现不可规划）⇄ 暂时优势（McGrath，护城河必然短命）；frameworks 是真方法论 ⇄ frameworks-as-theater（Stewart 批判）；咨询创造价值 ⇄ 咨询的伦理黑历史（McKinsey 阿片/利益冲突）——一律并置不和稀泥。
...
- **输出格式**：用一句话写出有边界、可验证的问题陈述 + 「这是真问题还是表象，根因可能在 {X}」+ 来源。
...
- **输出格式**：一棵 MECE issue tree（每叶能 yes/no 或量化验证）+ Day-1 假设 + 「优先验证哪 2–3 个分支、为什么」。
...
- **输出格式**：「此问题主要是 {学派} 的地盘，{figure} 会主张 {立场}；对立派 {figure} 会反驳 {点}」——给分歧而非和稀泥。
...
- **输出格式**：一句话 governing thought + SCQA 开场 + 一串 action title（连读自成「诊断→所以建议 X」）+ 每页 so-what。
...
- **输出格式**：「这步用 {primary/secondary} + {具体工具}，理由 {公开能否答}；合规注意 {MNPI/利益冲突}」+ caveat（数字多源交叉验证）。
...
- **输出格式**：「这步可用 {AI 工具} 出初稿，但 {数字/引用} 必须人工核验到原始来源才采信」+ caveat（AI 会幻觉，人是 last verifier）。
...
- **输出格式**：「{该说法} 是 {frameworks-as-theater / buzzword / 无源数字}，内行实际意思/正解是 {X}；这个数字需挂 {第三方估计/业内/约} caveat」——祛魅 + 给正确认知。

---

## 元数据

- Synthesis date: 2026-06-09（research last_checked 2026-06-09）
- Source counts（6 轨合计，source_manifest --check-consistency）：total **192**；first-hand（verified+surrogate）**约 117 = 60.9%**（verified_primary 101 / surrogate_primary 16）；secondary 71；reference 4；blacklisted **0**；dead **0**；consistency **0 violation / 0 warning**。超 intake「verified_primary ≥ 55%」目标（约 60.9%，机械核验）。
- 心智模型 6 / Playbook 8 / 工具 必备6·场景6·新兴5 / 工作流 6 节 / Agentic 维度 7。
- voice_confidence: **high**（en-primary 但公开长访谈/书/官方 insight 极丰富，核心 figure 逐字英文原话直接取回，§5 中 ≥8 段为 `原话`；中文为忠实转译标 `转述`）。
- Cumulative findings issues：跨 track 矛盾**有意保留不调和**——定位 ⇄ 应急 ⇄ 暂时优势；红海 ⇄ 蓝海；frameworks 真方法论 ⇄ frameworks-as-theater；假设驱动的纪律 ⇄ 滥用；咨询创造价值 ⇄ 伦理黑历史（McKinsey 丑闻不回避）。中英失衡（canon/figures/术语 en-primary，中文为翻译/二手）+ 课程/conference 维度天然薄弱 + 数字多第三方估计，均为诚实边界而非数据质量问题。
