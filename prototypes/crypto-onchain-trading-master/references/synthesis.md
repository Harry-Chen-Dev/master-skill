# 加密 / 链上交易 (Crypto & On-chain Trading) Master OS — Synthesis

> Phase 2 提炼结果。Phase 3 (skill writeup) 直接消化本文件。
> 调研日期 2026-06-20，6 轨 research notes（256 条 manifest 信息源 / 14 figures / 7 workflows / 73 术语 / 35 canon 条目）。
> 本行第一性风险 = **信号质量（水分极高：付费喊单 / 杀猪盘 / 幸存者偏差 / 利益冲突）**，全篇心智模型与 playbook 都内嵌「先 de-bias 再行动」。
...
- **输出格式**：一句话定位「当前处于流动性扩张/收缩象限」+ 据此给出总风险敞口建议（加/中性/降）。
...
- **输出格式**：红旗清单（解锁砸盘风险/庄持仓/可增发/貔貅）+ 结论「可投 / 观望 / 避开」+ 关键不确定项。

### 9.3 资金流与聪明钱
- **看什么**：交易所净流入/流出、巨鲸地址动向、聪明钱标签与早期买家、是否存在反向狩猎结构。
- **在哪看**：Nansen（smart money 标签）、Arkham（实体标注）、CryptoQuant（交易所流向）。
- **输出格式**：「资金在进还是出」+ 信号可信度（标签需多源交叉）+ 被反向狩猎的风险提示。

### 9.4 叙事 / 板块轮动定位
- **看什么**：当前主导叙事、attention 流向、板块强弱与轮动、该叙事是否已成公共知识（晚期）。
- **在哪看**：Twitter/X（Crypto Twitter）、Messari Theses（年度叙事）、Dexscreener trending。
- **输出格式**：「当前 meta 是什么 + 处于启动/加速/过剩哪段 + 我是否在接最后一棒」+ 下一轮候选叙事。

### 9.5 衍生品 / 市场结构
- **看什么**：资金费率、未平仓 OI、清算热图、多空比、杠杆拥挤度。
- **在哪看**：Coinglass（费率/清算/OI）、各交易所衍生品数据。
- **输出格式**：「杠杆拥挤度 + 大概率挤压方向（多杀多/空杀空）+ 清算价地图」，标明数据时点（随市场快速变）。

### 9.6 信息源利益冲突核查
- **看什么**：信号发布者的持仓 / 基金 / 返佣 / 项目关系 / 历史准确率，是否 talk-his-book。
- **在哪看**：发布者公开披露、其链上地址（Arkham）、ZachXBT 等反诈取证。
- **输出格式**：「该信号的立场偏向（看多/看空谁受益）+ 去偏后的可信度权重」，杀猪盘特征命中即拉黑。

### 9.7 风控与仓位前置
- **看什么**：单笔 1R 风险、与现有持仓的相关性、最大回撤承受、自托管安全状态（授权/钱包分层）。
- **在哪看**：自己的交易日志、Revoke.cash（授权）、钱包分层方案。
- **输出格式**：「仓位大小 + 止损/失效位 + 钱包隔离方案（热/冷/可归零额）」，没有失效定义则结论为「不开仓」。

---

## 元数据
- Synthesis date: 2026-06-20
- Source counts: 256 manifest 条目（verified_primary 161 ≈ 62.9% 机械统计 / secondary 81 / reference 14）；14 figures / 27 tools / 7 workflows / 35 canon / 73 术语。
- 一手率：verified_primary 约 62.9%（本库机械统计，≥50% 门槛）。
- Cumulative findings issues：中文圈独立一手交易思想缺位（已写入诚实边界）；AI 交易 agent 维度信号薄弱（可信度 low）。无未解决的跨 track 硬矛盾。
- voice_confidence: high（对话样本库 10 段计入，原话占比 ≥ 60%）。
