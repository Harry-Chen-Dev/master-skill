# 个人投资理财 (Personal Investing & Financial Planning) · Master OS (Phase 2 Synthesis)

> 行业本质：个人/家庭**自主**管理投资组合与长期财务规划的认知操作系统——不是「教你买哪只会涨」，是「怎么搭一个低成本、税务高效、自动化、几十年管得住手的组合」。
> 一句话定性：**个人投资理财不是「选到会涨的标的」，是「四派共享的『配置 + 容器 + 成本 + 纪律』决定 95% 成败，价值/被动/技术/趋势之争只决定『选品』那一小步」——资深 = 做得更少，不是更多。**
> 研究日期：2026-06-04。locale = global（英文正典绝对主导 ≈9:1 + zh-CN 散户实践层）。一手率 63.9%（144 源：92 verified_primary / 48 secondary / 4 reference / **0 黑名单 / 0 死链**）。
> **非投资建议**：本文是认知 OS（这一行「怎么想 / 怎么操作」），不构成任何具体证券、买卖时点或配置比例的投资建议；过往业绩不预示未来。
...
## 9. Agentic Protocol（拿到个人投资任务时，按这些维度先做功课再动手）

> 6 个维度互斥，各产出不同结论。Phase 3 `cli_writer.py` 把每个维度编排进 `cli/protocol/agentic.sh` 一个分节。
...
- **输出格式**：一句话风险画像 + 时间轴 + 「跌 X% 我也不卖」的明确底线（写进 IPS）。
...
- **输出格式**：按顺序排好的「先填哪个容器」清单 + 当年限额数字（标地域+年份）。
...
- **输出格式**：目标配置 % + 再平衡带宽（5/25 或日历）+ 一句话风险预期（最大历史回撤量级）。
...
- **输出格式**：选定「被动宽基 / 主动价值 / 技术 / 趋势」分支 + 若主动则写下 edge 理由（答不出=回被动）。
...
- **输出格式**：加权 ER 数字 + 可省的成本/税清单（asset location 调整、TLH 标的、该不该上 robo）。
...
- **输出格式**：成文 IPS 要点 + 「跌 X% 我继续投/再平衡」预案 + 已自动化的步骤清单（把意志力省给「管住手」）。

---

## 元数据
- Synthesis date: 2026-06-04
- Source counts: 144 total（92 verified_primary / 48 secondary / 4 reference / 0 blacklisted / 0 dead），一手率 63.9%；六轨 manifest：T01 figures 27 / T02 tools 35(53 行含跨引) / T03 workflows 23 / T04 canon 26 / T05 sources 43 / T06 glossary 24。
- 行业类型：global classic-finance OS（公开材料极丰，academic+vertical 混合）→ 候选数门槛 ≥12，实际心智模型候选 ~19、playbook 候选 ~10，远超阈值。
- Cumulative findings issues：无跨 track 硬矛盾。结构性边界（非矛盾）：① canon 英文 9:1 主导；② 技术/趋势派 figures 薄（仅 Covel 可调研）+ 学术否定；③ zh-CN 散户一手沉在被排除渠道；④ 数字层（税优限额/费率/SPIVA）高衰减须年度刷新。四派分歧（主动vs被动等）按 disputed 保留不统一。
