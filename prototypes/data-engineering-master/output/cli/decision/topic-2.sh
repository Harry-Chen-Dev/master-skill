...
explain() {
  cat <<'EOT'
这个决策助手把 分区 主题下的 playbook 规则组合成交互树。

基于 2 条规则:
  • 如果 要设计任何写入 / 转换任务 → 则 : 则 必须对目标分区幂等 — 用 MERGE/upsert/INSERT OVERWRITE 分区而非盲 append
  • 如果 云数仓 / 集群账单异常 → 则 : 则 查四元凶 — SELECT * 全表扫 / 全量重算 (该增量) / 小文件过多 / 缺分区裁剪与谓词下推, 用
...
declare -a RULE_ACTIONS=(
  ": 则 必须对目标分区幂等 — 用 MERGE/upsert/INSERT OVERWRITE 分区而非盲 append, 给增量模型定主键, 让回填只是重跑历史分区"
  ": 则 查四元凶 — SELECT * 全表扫 / 全量重算 (该增量) / 小文件过多 / 缺分区裁剪与谓词下推, 用分区+聚簇+物化+按需暂停+查询审计治理"
)
