...
基于 1 条规则:
  • 如果 一段转换逻辑能用 SQL 表达 → 则 : 则 默认 dbt/SQLMesh (模块化+测试+血缘+版本控制), 只有重度非 SQL 逻辑 (复杂 ML 特征/
...
declare -a RULE_ACTIONS=(
  ": 则 默认 dbt/SQLMesh (模块化+测试+血缘+版本控制), 只有重度非 SQL 逻辑 (复杂 ML 特征/图计算/自定义 UDF) 才落 Spark/Polars"
)
