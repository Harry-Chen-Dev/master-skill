# 小红书 AI 自媒体博主 CLI

把 小红书 AI 自媒体博主 master skill 的认知 OS 物化成 bash 工具。
不替代 SKILL.md（思维顾问），是它的「执行端」：交互问询 → 应用 playbook / protocol → 输出结构化报告。

## 用法

所有脚本支持 `--help` / `--explain` / `--dry-run` / `--json` 四个标准 flag。

```bash
# 拿到新问题时, 按 N 个研究维度做功课
./protocol/agentic.sh

# 决策树评估 (基于 playbook)
./decision/topic-1.sh
# SOP 走查 (workflow)
./workflow/sop-seo.sh

# 看背后的心智模型 / playbook (不交互)
./protocol/agentic.sh --explain
```

## 脚本清单

| 脚本 | 作用 |
|------|------|
| `protocol/agentic.sh` | Agentic Protocol (6 维度) — 拿到新问题时按这一行的研究维度做功课 |
| `decision/topic-1.sh` | 案例 决策树 (8 条规则) |
| `decision/topic-2.sh` | 工具 决策树 (1 条规则) |
| `decision/topic-3.sh` | 封面 决策树 (1 条规则) |
| `workflow/sop-seo.sh` | 端到端内容生产 SOP（核心 · 选题→标题封面→正文→制作→标签SEO→发布→复盘） SOP 走查 |
| `workflow/workflow-1.sh` | 爆款拆解工作流（对标一条爆款→拆出可复用结构） SOP 走查 |
| `workflow/workflow-2.sh` | 不同内容类型的生产链差异（图文干货 / 测评种草 / 口播教程 / 资讯合集） SOP 走查 |
| `workflow/ai-ai.sh` | AI 出稿 → 人工重写终稿（防 AI 味/防限流 · 核心高杠杆环） SOP 走查 |
| `workflow/0-1.sh` | 起号 0→1（垂直度/养号/前几条/破播放） SOP 走查 |
| `workflow/workflow-3.sh` | 数据复盘迭代（收藏率/完播/互动） SOP 走查 |
| `workflow/workflow-4.sh` | 合规 / 质检环（发布前一遍过 · 三道闸） SOP 走查 |

## 设计与生成

CLI 子树由 `tools/cli_writer.py` 自动从 `references/synthesis.md` (Section 2 Playbook + Section 9 Agentic Protocol) 和 `references/research/03-workflows.md` 生成。

完整 spec 在 master-skill 仓库 `references/cli-spec.md`。

## 重新生成

如果 synthesis.md 或 03-workflows.md 更新了, 重跑:

```bash
python3 <master-skill>/tools/cli_writer.py emit \
  --skill-dir <this-skill-dir> \
  --synthesis references/synthesis.md \
  --workflows references/research/03-workflows.md \
  --industry-cn "小红书 AI 自媒体博主"
```
