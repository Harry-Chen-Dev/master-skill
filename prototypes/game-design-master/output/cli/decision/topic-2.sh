...
explain() {
  cat <<'EOT'
这个决策助手把 玩家 主题下的 playbook 规则组合成交互树。

基于 3 条规则:
  • 如果 你觉得「这个设计很明显能行」 → 则 : 则 用真实玩家 playtest 验证, 看而不说、记卡点不记意见, 「你不是你的玩家」; 玩家的症状 (卡在哪) 
  • 如果 玩家在某处大量流失 / 觉得太难或太无聊 → 则 : 则 检查挑战是否匹配技能 (心流通道), 太难加辅助/降难度, 太易加深度/分流; 用动态难度或可选难度照顾不同玩家
  • 如果 要教玩家一个新机制 → 则 : 则 用 teach-by-doing (在安全的玩法情境里让玩家试, 而非堆文字教程), 一次教一个, 即时反馈; 
...
declare -a RULE_ACTIONS=(
  ": 则 用真实玩家 playtest 验证, 看而不说、记卡点不记意见, 「你不是你的玩家」; 玩家的症状 (卡在哪) 永真, 玩家的诊断 (该怎么改) 常错"
  ": 则 检查挑战是否匹配技能 (心流通道), 太难加辅助/降难度, 太易加深度/分流; 用动态难度或可选难度照顾不同玩家"
  ": 则 用 teach-by-doing (在安全的玩法情境里让玩家试, 而非堆文字教程), 一次教一个, 即时反馈; FTUE 的目标是让玩家「做中学会」不是「读完说明书」"
)
