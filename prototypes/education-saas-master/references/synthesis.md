# Education SaaS — 行业 OS Synthesis

> 调研日期: 2026-05-26 | locale: global (EN primary ~60%, zh-CN ~35%) | profile: practitioner
...
- **怎么查**: Phil Hill / Edutechnica 年度 LMS 市场份额报告; HolonIQ EdTech 融资数据; ListedTech 技术部署追踪
- **判断锚**: 目标市场 top 3 玩家份额之和 > 70% 说明寡头化 — 走差异化切入; < 50% 说明碎片化 — 走标准化整合

### 9.2 合规与数据治理

- **看什么**: 目标地域的教育数据合规要求 (FERPA/COPPA/GDPR/PIPL/等保 2.0)
- **怎么查**: studentprivacy.ed.gov (美); gdpr.eu + DPA guidance (欧); 个保法 + 等保 2.0 原文 (中)
- **判断锚**: 是否需要数据驻留? 用户包含 < 13 岁儿童? 是否需要 SOC 2 / 等保认证? 任一为"是"则合规工作在 MVP 前必须完成

### 9.3 标准互操作评估

- **看什么**: 目标 LMS/SIS 生态支持哪些标准 (LTI/SCORM/xAPI/OneRoster)
- **怎么查**: 1EdTech Certified Product Directory; Canvas/Moodle/Brightspace 官方集成文档
- **判断锚**: 目标客户 LMS 是否支持 LTI 1.3? 如果还在 LTI 1.1 — 需要降级兼容或等 sunset

### 9.4 教师 Adoption 策略

- **看什么**: 目标用户群体的技术接受度 + 痛点优先级
- **怎么查**: EDUCAUSE Learning Initiative surveys; 教师论坛 / Canvas Community 讨论; 种子教师深度访谈
- **判断锚**: 教师的 #1 痛点是什么? 如果是"太多工具"→ 做集成; 如果是"没时间学"→ 做极简 UI; 如果是"看不到效果"→ 做 learning analytics dashboard

### 9.5 采购周期与预算节奏

- **看什么**: 目标客户的预算审批周期 + 决策链
- **怎么查**: EDUCAUSE IT Issues Survey; 高校 CFO/CIO 公开演讲; K-12 district budget calendar
- **判断锚**: 采购窗口在哪个月? 决策者是 CIO/Provost/教务处长? pilot 时长 = 1 学期还是 1 学年? 错过预算窗口意味着等 12 个月

### 9.6 产品定价与商业模型

- **看什么**: 同类产品的定价结构 + NRR benchmark
- **怎么查**: SaaStr benchmark reports (Jason Lemkin); Tomasz Tunguz SaaS metrics; 公开 pricing page 对比
- **判断锚**: per-student vs per-institution vs per-module? freemium 还是 enterprise-only? NRR 目标 > 110%? CAC payback < 18 月?

### 9.7 AI 能力评估

- **看什么**: AI 功能在教育场景的实际效果 vs 营销承诺
- **怎么查**: 独立 RCT 研究 (少); Khanmigo/ALEKS 公开效果数据; EdSurge AI coverage; EDUCAUSE AI landscape report
- **判断锚**: 有独立第三方效果验证? 还是只有厂商自报? Bloom 2-sigma 的现实锚 (d≈0.3-0.5σ) 作为效果预期上限
