# 德州扑克策略 (Texas Hold'em / NLHE) Master OS — Synthesis

> Phase 2 提炼结果。Phase 3 (skill writeup) 直接消化本文件。
> 输入:`references/research/01-06.md`(figures / tools / workflows / canon / sources / glossary,共 198 sources,first-hand 约 78%)。
> locale=global / en-primary:本行业一手 canon、figures、工具、术语**几乎 100% 英文**;中文世界为社区转述/翻译,且多落黑名单。诚实边界第 8 节明示。
...
> 这一行人拿到一个新问题(「我该怎么打/学/选」)时,默认按以下维度做功课再答。7 个维度互斥,各产出不同结论。
...
- **输出格式**:1 句定性「本问题属 {cash / MTT 泡沫 / MTT 深筹},{用 / 不用} ICM,MDF {适用 / 应主动偏低}」。
...
- **输出格式**:1-2 句「均衡基线 = {频率/尺度},关键 heuristic = {一句可记忆规则},例外条件 = {…}」。
...
- **输出格式**:1 句「{有 / 无}足够样本支撑偏离;若有,用 node-lock/population baseline 朝 {方向} 剥削;若无,打 GTO 基线」。
...
- **输出格式**:1 句「换桌/选房收益 {高 / 低};建议 {坐哪 / 换房 / 拿 rakeback};rake 对本级别边际赢率影响 {大 / 小}」。
...
- **输出格式**:1 句「当前应打 {级别};{升级线/降级 stop-loss};{是否 shot-take};缓冲按 {职业/业余 + winrate 置信度} 调」。
...
- **输出格式**:1 句「状态 {A/C-game};{继续 / 降级减桌 / 止损离场};tilt 处置 = {在阈值前注入逻辑 / 已过阈值应离场}」。
...
- **输出格式**:1 句「本牌房 {允许/禁} HUD、{匿名/非匿名};复盘/leak 重心放 {数据库 / 纯 solver};合规红线 = 离线 solver 学习 OK、实时 RTA 禁」。
...
- **Source counts**: 6 轨共 198 sources(01-figures 48 / 02-tools 33 / 03-workflows 28 / 04-canon 31 / 05-sources 33 / 06-glossary 25);first-hand(verified_primary + surrogate_primary)约 78%(manifest 口径),bucket-consistency violations 0。
- **行业类型**: competitive mind-sport / 极 codifiable(global, en-primary);**非冷僻**(figures 13 / 必读书 13 + 论文 8 / 工具 15 / 术语 40 / workflow 6 / sources 33,六维全部远超 floor)。
- **Cumulative findings issues**: 无未解决的跨 track 矛盾。已知边界:中英失衡(en-primary,中文一手结构性缺位)+ 闭源工具 benchmark 多为厂商自报 + 在线最强牌手隐性知识不可蒸馏,均在第 8 节诚实边界明示。
- **sub_skills**: skip(intake.skip_sub_skills=true,通用 description 未要求 figures 做成 sub-skill)。
